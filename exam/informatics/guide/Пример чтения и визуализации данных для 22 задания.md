...
def get_max_path_in_graph(graph: defaultdict, start: int = 0):
    ...
    # 3. Инициализация: dp[v] = [предыдущая вершина, расстояние]
    dp = defaultdict(lambda: [None, float('-inf')])
    dp[start] = [None, 0]  # стартовая вершина — без предшественника, расстояние 0

    # 4. Обновление расстояний в топологическом порядке
    for u in topo:
        if dp[u][1] == float('-inf'):
            continue  # не достижима из start
        for v, w in graph.get(u, []):
            new_dist = dp[u][1] + w
            if new_dist > dp[v][1]:
                dp[v] = [u, new_dist]  # запоминаем предшественника и новое расстояние
